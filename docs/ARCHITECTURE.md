...
- Once a cluster expands, each campground uses one split MapLibre symbol: the left half shows the recent camper-report state and the right half shows today's modeled forecast. Either half is gray when its respective signal is unavailable. A white center divider and outline keep the two independent signals visually distinct.
