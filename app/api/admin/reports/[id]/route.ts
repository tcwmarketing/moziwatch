...
export async function PATCH(
  ...
  await sqlClient.begin(async (tx) => {
    ...
    await tx`INSERT INTO admin_audit_logs (actor_id, action, target_type, target_id, details) VALUES (${admin.user.id}, 'moderate_report', 'report', ${id}, ${tx.json({ from: current[0].moderation_status, to: parsed.data.status })})`;
